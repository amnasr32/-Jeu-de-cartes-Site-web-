...
<?php
    ...
    if(isset($_POST['submit_check4'])){
        ...
        if (strlen($_POST["reponse_carte_cuisine"][$i]) !== 0) {
            ?>
            <p> Votre réponse à la question <?php echo $i; ?> est la suivante: </br><?php echo $_POST["reponse_carte_cuisine"][$i]; 
       ...
       $n=0;
    	...
    	while($ligne=mysqli_fetch_assoc($res)){
    		...
    		if((htmlspecialchars($ligne['reponse']))==$_POST["reponse_carte_cuisine"][$n]){
				//echo " rep correct";
    			$score=$score+1;
    		...
    		$_SESSION["reponse_carte_cuisine"]=$_POST["reponse_carte_cuisine"][$n];
...
//se connecter à notre base de donnée:
			...
			$roq="SELECT * FROM cartes_cuisine" ;
...
<!--Le formulaire de la réponse-->
	<form method="POST" action="cartes_cuisine.php">
	<p>Votre réponse:</p>
	<input type ="text" name="reponse_carte_cuisine[]" placeholder="Indiquez votre réponse" > 
...
echo"<hr>";}
			?>
		 <input type="submit" size="20"  value="vérifier" name="submit_check4">
			</form>
