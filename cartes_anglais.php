...
<body>
<hr/>
<h2> Bienvenue dans les cartes d'anglais <br></h2>
...
<?php
    ...
    if(isset($_POST['submit_check'])){
		...
		//selectionner tt dans ma table
			$req="SELECT * FROM cartes_anglais" ;
			
			$res=mysqli_query($connexion,$req);
			//rows est le nombre de lignes
			$rows_2=mysqli_num_rows($res);?>
		</br>
        ...
        if (strlen($_POST["reponse"][$i]) !== 0) {
            ?>
            <p> Votre réponse à la question <?php echo $i; ?> est la suivante: </br><?php echo $_POST["reponse"][$i]; 
       ...
       $n=0;
    	...
    	while($ligne=mysqli_fetch_assoc($res)){
    		...
    		if((htmlspecialchars($ligne['reponse']))==$_POST["reponse"][$n]){
				//echo " rep correct";
    			$score=$score+1;
    		...
    		$_SESSION["reponse"]=$_POST["reponse"][$n];
...
//se connecter à notre base de donnée:
			...
			$roq="SELECT * FROM cartes_anglais" ;
...
<!--Le formulaire de la réponse-->
	<form method="POST" action="cartes_anglais.php">
	<p>Votre réponse:</p>
	<input type ="text" name="reponse[]" placeholder="Indiquez votre réponse" > 
...
echo"<hr>";}
			?>
		 <input type="submit" size="20"  value="vérifier" name="submit_check">
			</form>
